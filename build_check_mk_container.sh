...
#CMK_VERSION=1.6.0i1
#CMK_EDITION=raw
#CMK_PASSWORD='ralrox22'
#CMK_SITE_ID=hts
#MAIL_RELAY_HOST='relay.companynet.org'
#CMK_LIVESTATUS_TCP=on -p 6557:6557

# Download the file to your docker node and execute the load command:
#curl -O https://mathias-kettner.de/support/${CMK_VERSION}/check-mk-${CMK_EDITION}-docker-${CMK_VERSION}.tar.gz
#docker load -i check-mk-${CMK_EDITION}-docker-${CMK_VERSION}.tar.gz
#
## After a successful import you can start creating containers using the
## image `checkmk/check-mk-enterprise:1.5.0p5`.

docker run -dit \
    -e CMK_PASSWORD='ralrox22' \
    -e CMK_SITE_ID=hts \
    -e MAIL_RELAY_HOST='relay.companynet.org' \
    -e CMK_LIVESTATUS_TCP=on \
    -p 6557:6557 \
    -p 8080:5000 \
    --tmpfs /omd/sites/hts/tmp \
    -v /omd/sites \
    --name check_mk \
    --restart always \
    rlaney/check-mk-raw:1.5.0p5
...
# uvwxy/checkmk                 Unofficial docker container of Check_MK         5                                       [OK]
# checkmk/check-mk-raw          Official Check_MK Raw Edition Container         2
# eeacms/check-mk-agent         CheckMK Agent                                   1                                       [OK]
# xforty/checkmk                                                                0                                       [OK]
# craigwillis/checkmk                                                           0
# craigwillis/checkmk-wt                                                        0
# cuong2k/checkmkv1                                                             0
# pnnlhep/checkmk-client        Monitoring                                      0                                       [OK]
# str1k3r/rps-checkmk           ubuntu with xinetd and check-mk-agent packag…   0
# pjmorr/docker-checkmk                                                         0
# ndslabs/checkmk-kube          Docker image for checkmk agent with basic Ku…   0                                       [OK]
# majorem/checkmkagent                                                          0
# aalbng/checkmk                Fully functional check-mk image. Run and sta…   0
# bl0nd1n/checkmk                                                               0
# pjmorr/checkmk                                                                0
# freenas/checkmk-agent         Check_MK Agent to collect and allow reportin…   0                                       [OK]
# craigwillis/checkmk-kube                                                      0
# hbrsnetlab/servmgmt-checkmk   checkmk for teaching purposes                   0
# iduvlad/checkmk                                                               0
# freyp567/docker-checkmk       container with Check-MK                         0
